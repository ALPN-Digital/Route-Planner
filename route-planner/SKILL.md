...
8. **Never anchor a route on a town or place name.** Geocoding a settlement returns its *centre*, so forcing it as a via-point makes the route dive into the town centre and back out, a pointless dog-leg. Do not use place-name geocodes as routing via-points. Geocode only the genuine start, the destination, and explicitly requested stops. To bias the line toward a feature (coast, ridge, valley, a named road or cycleway), use a coordinate that sits *on that feature's road/path* in the direction of travel, taken from the map, not a town centroid. After routing, inspect the line: if it darts into somewhere and back along nearly the same road, remove that via and re-route. Pass through towns by letting the engine route through them, never by pinning their centre. **`route.py` self-checks this:** every build reports `retrace_pct` (how much of the route re-covers earlier ground) and `dead_end_spurs` (tight out-and-back pokes, with tip coordinates), and prints a WARNING when more than one spur is found. After each build, read these — more than one spur, or any spur you did not intend, means a via sits on a dead-end lane: remove it and re-route. `retrace_pct` near 0 is a clean loop; ~50 is a pure out-and-back; if the user asked for a loop but `retrace_pct` is high, the outbound and return share a corridor and you need a genuinely different return via, not a cosmetic one.
